...
let PlayersController = class PlayersController {
    ...
    async create(createPlayerDto, res) {
        try {
            ...
            return res.status(common_1.HttpStatus.OK).json(player);
        }
        catch (error) {
            if (error.status === 409) {
                return res.status(common_1.HttpStatus.CONFLICT).json({ code: 409, message: error.message });
            }
            return res.status(common_1.HttpStatus.BAD_REQUEST).json({ code: 400, message: 'Invalid player ID' });
        }
    }
};
